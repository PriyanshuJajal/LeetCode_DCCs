    ...
    public int maxProductPath(int[][] grid) {
        ...
        for (int i = 1; i < m; i++) {
            for (int j = 1; j < n; j++) {
                ...
                long candidate1 = val * maxPrev;
                long candidate2 = val * minPrev;

                maxDP[i][j] = Math.max(candidate1, candidate2);
                minDP[i][j] = Math.min(candidate1, candidate2);
            }
        }
