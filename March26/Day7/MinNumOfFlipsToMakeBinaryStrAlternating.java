...
class Solution {
    public int minFlips(String s) {
        ...
        int i = 0, j = 0;
        while (j < 2 * n) {
            char expChar1 = (j % 2 == 1) ? '1' : '0';
            char expChar2 = (j % 2 == 1) ? '0' : '1';

            if (s.charAt(j % n) != expChar1) f1++;
            if (s.charAt(j % n) != expChar2) f2++;

            if (j - i + 1 > n) {
                expChar1 = (i % 2 == 1) ? '1' : '0';
                expChar2 = (i % 2 == 1) ? '0' : '1';

                if (s.charAt(i % n) != expChar1) f1--;
                if (s.charAt(i % n) != expChar2) f2--;

                i++;
            }

            if (j - i + 1 == n) {
                flips = Math.min(flips, Math.min(f1, f2));
            }
