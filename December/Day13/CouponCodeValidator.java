...
class Solution {
    public List<String> validateCoupons(List<String> code, List<String> businessLine, List<Boolean> isActive) {
        ...
        // Valid categories
        Set<String> validCat = new HashSet<>(Arrays.asList("electronics", "grocery", "pharmacy", "restaurant"));
        
        // Collect valid coupons as (businessLine, code)
        List<Pair> valCoup = new ArrayList<>();
        
        for (int i = 0; i < n; i++) {
            String c = code.get(i);
            String b = businessLine.get(i);
            boolean active = isActive.get(i);
            
            if (!active) continue;
            if (!validCat.contains(b)) continue;
            if (c.isEmpty()) continue;
            
            boolean valCode = c.chars().allMatch(ch -> Character.isLetterOrDigit(ch) || ch == '_');
            if (!valCode) continue;
            
            valCoup.add(new Pair(b, c));
        }
        
        // Order map for business lines
        ...
        valCoup.sort((a, b) -> {
            int cmp = Integer.compare(order.get(a.business), order.get(b.business));
            if (cmp != 0) return cmp;
