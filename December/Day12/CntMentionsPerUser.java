...
class Solution {
    static class Event {
        int timestamp;
        int type; // 0 = OFFLINE, 1 = MENTION
        int idx;

        Event(int timestamp, int type, int idx) {
    ...
    public int[] countMentions(int numberOfUsers, List<List<String>> events) {
        ...
        for (int i = 0; i < events.size(); i++) {
            ...
            int type = events.get(i).get(0).equals("OFFLINE") ? 0 : 1;
            sorted.add(new Event(time, type, i));
        }

        Collections.sort(sorted, (a, b) -> {
            if (a.timestamp != b.timestamp) return Integer.compare(a.timestamp, b.timestamp);
        ...
        for (Event ev : sorted) {
            ...
            List<String> ogEvent = events.get(ev.idx);

            if (ev.type == 0) { 
                int userId = Integer.parseInt(ogEvent.get(2));
                online[userId] = currTime + 60;
            } else {
                String mentionStr = ogEvent.get(2);
