...
class Solution {
    public int minimumTeachings(int n, int[][] languages, int[][] friendships) {
        Set<Integer> usersCantTalk = new HashSet<>();
        ...
        Map<Integer, Integer> langCnt = new HashMap<>();
        for (int user : usersCantTalk) {
            for (int lang : languages[user]) {
                langCnt.put(lang, langCnt.getOrDefault(lang , 0) + 1);
            }
        }

        int maxKnown = 0;
        for (int cnt : langCnt.values()) {
            maxKnown = Math.max(maxKnown , cnt);
        }
