...
public:
    double maxAverageRatio(vector<vector<int>>& classes, int extraStudents) {
        auto calculateGain = [](double pass , double total) -> double {
        ...
        for (const auto& c : classes) {
            ...
            pq.push({calculateGain(pass , total) , {c[0] , c[1]}});
        }
        ...
        while(!pq.empty()) {
            pair<int , int> classData = pq.top().second;
            pq.pop();
            totalRatioSum += static_cast<double>(classData.first) / classData.second;
        }
