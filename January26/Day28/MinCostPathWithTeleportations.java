...
class Solution {
    public int minCost(int[][] grid, int k) {
        ...
        // Sort cells by their grid values
        pts.sort((p1, p2) -> Integer.compare(grid[p1[0]][p1[1]] , grid[p2[0]][p2[1]]));

        int[][] costs = new int[m][n];
        for (int[] row : costs) Arrays.fill(row, Integer.MAX_VALUE);

        // Relaxation up to k times
        for (int t = 0; t <= k; t++) {
            int minCst = Integer.MAX_VALUE;
            for (int i = 0, j = 0; i < pts.size(); i++) {
                minCst = Math.min(minCst, costs[pts.get(i)[0]][pts.get(i)[1]]);

                if (i + 1 < pts.size() &&
                    grid[pts.get(i)[0]][pts.get(i)[1]] == grid[pts.get(i + 1)[0]][pts.get(i + 1)[1]]) {
                    continue;
                }

                for (int r = j; r <= i; r++) {
                    costs[pts.get(r)[0]][pts.get(r)[1]] = minCst;
                }
                j = i + 1;
            }
