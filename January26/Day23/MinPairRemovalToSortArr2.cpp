...
public:
    ...
    int minimumPairRemoval(vector<int>& nums) {
        ...
        while(badPairs > 0) {
            ...
            if (leftFirst >= 0) {
                minPairSet.erase({t[leftFirst] + t[first] , leftFirst});
                minPairSet.insert({t[leftFirst] + t[first] + t[second] , leftFirst});
            }

            if (rightSecond < n) {
                minPairSet.erase({t[second] + t[rightSecond] , second});
                minPairSet.insert({t[first] + t[second] + t[rightSecond] , first});
