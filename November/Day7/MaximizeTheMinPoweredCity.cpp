...
class Solution {
public:
    bool isEnough(vector<int>& stations , int r , long long targetPow , int addedSt) {
        int n = stations.size();
        long long pow = accumulate(begin(stations) , begin(stations) + r , 0LL);
        vector<int> addPower(n , 0);
        
        for (int i = 0; i < n; i++) {
            if (i + r < n) pow += stations[i + r];
            if (pow < targetPow) {
                long long needed = targetPow - pow;
                if (needed > addedSt) return false;

                addPower[min(n - 1 , i + r)] += needed;
                pow = targetPow;
                addedSt -= needed;
            }   

            if (i - r >= 0) pow -= stations[i - r] + addPower[i - r];
        }
        return true;
    }
    ...
    long long maxPower(vector<int>& stations, int r, int k) {
        long long l = 0;
        long long h = accumulate(begin(stations) , end(stations) , 0LL) + k;
        long long res = 0;

        while(l <= h) {
            long long mid = l + (h - l) / 2;
            if (isEnough(stations , r , mid , k)) {
                res = mid;
                l = mid + 1;
            }
            else h = mid - 1;
