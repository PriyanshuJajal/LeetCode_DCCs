...
class DSU {
    ...
    public void unionSet(int i, int j) {
        ...
        if (!onlineNodes[iParent].isEmpty()) {
            onlineNodes[iParent].addAll(onlineNodes[jParent]);
            onlineNodes[jParent].clear();
        }
    }

    public void goOffline(int x, boolean[] isOnline) {
        if (!isOnline[x]) return;
        isOnline[x] = false;

        int root = find(x);
        onlineNodes[root].remove(x);
    }

    public int getMinOnline(int x, boolean[] isOnline) {
        if (isOnline[x]) return x;

        int root = find(x);
        if (onlineNodes[root].isEmpty()) return -1;
        return onlineNodes[root].first();
    }
}
...
class Solution {
    public int[] processQueries(int c, int[][] connections, int[][] queries) {
        ...
        for (int[] q : queries) {
            ...
            else result.add(dsu.getMinOnline(x , isOnline));
        }
