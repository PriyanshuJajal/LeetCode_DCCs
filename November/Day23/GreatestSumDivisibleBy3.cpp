...
public:
    int maxSumDivThree(vector<int>& nums) {
        ...
        long long remove = 1e18;
        if (rem == 1) {
            if (r1Min1 != INF) remove = min(remove , (long long)r1Min1);
            if (r2Min2 != INF) remove = min(remove , (long long)r2Min1 + r2Min2);
        }
        else {
            if (r2Min1 != INF) remove = min(remove , (long long)r2Min1);
            if (r1Min2 != INF) remove = min(remove , (long long)r1Min1 + r1Min2);
        }
