...
class Solution {
    private boolean isValid(int[] cards) {
        ...
        return isValid(a + b, c, d) || isValid(a - b, c, d) || isValid(a * b, c, d) || isValid(a / b, c, d) ||
               isValid(a, b + c, d) || isValid(a, b - c, d) || isValid(a, b * c, d) || isValid(a, b / c, d) ||
               isValid(a, b, c + d) || isValid(a, b, c - d) || isValid(a, b, c * d) || isValid(a, b, c / d);
    }

    private boolean isValid(double a , double b , double c) {
        return isValid(a + b, c) || isValid(a - b, c) || isValid(a * b, c) || (b != 0 && isValid(a / b, c)) ||
               isValid(a, b + c) || isValid(a, b - c) || isValid(a, b * c) || (c != 0 && isValid(a, b / c));
    }

    private boolean isValid(double a , double b) {
        return Math.abs(a + b - 24) <= 1e-5 ||
               Math.abs(a - b - 24) <= 1e-5 ||
               Math.abs(a * b - 24) <= 1e-5 ||
               (b != 0 && Math.abs(a / b - 24) <= 1e-5);
    }
